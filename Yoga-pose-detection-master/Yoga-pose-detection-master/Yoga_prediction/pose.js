...
var audio=null;
var twenty_frames=[];
var counts = {}; //object array counts.A/B
...
function setup() {
  ...
  video.size(width, height); //points
  video.hide(); //hidding video for html
  //image();
...
function gotResults(err, result) {
  ...
  if (result.confidencesByLabel) {
   ...
   if(twenty_frames.length>100){
    ...
    var aa = document.querySelector('#confidenceA').textContent = `${confidences['Saturanga'] ? 0 : confidences['Saturanga'] * 100} %`;
    //console.log(aa);
...
document.getElementById('smname').value = count1();
